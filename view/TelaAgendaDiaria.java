...
public class TelaAgendaDiaria extends JFrame {
    ...
    public TelaAgendaDiaria() {
        ...
        // BOTÃO 1: CONCLUIR (Verde)
        JButton btnConcluir = new JButton("✅ Concluir Sessão");
        btnConcluir.setBounds(280, 400, 160, 40);
        btnConcluir.setBackground(new java.awt.Color(144, 238, 144)); // Verde claro
        add(btnConcluir);

        // BOTÃO 2: CANCELAR (Vermelho)
        JButton btnCancelar = new JButton("❌ Cancelar");
        btnCancelar.setBounds(450, 400, 150, 40);
        btnCancelar.setBackground(new java.awt.Color(255, 182, 193)); // Rosa/Vermelho claro
        add(btnCancelar);

        // --- AÇÕES ---

        // Botão REALIZADO
        btnConcluir.addActionListener(e -> alterarStatusSessao("Realizado"));
        // Botão CANCELADO
        btnCancelar.addActionListener(e -> alterarStatusSessao("Cancelado"));
        // Botão Filtrar
    ...
    private void alterarStatusSessao(String novoStatus) {
        int linha = tabela.getSelectedRow();
        if (linha == -1) {
            JOptionPane.showMessageDialog(this, "Selecione um horário na tabela!");
            return;
        ...
        // Pega a sessão da memória
        Sessao s = listaSessoes.get(linha);

        // Pergunta de confirmação
        int confirmacao = JOptionPane.showConfirmDialog(this,
                "Mudar status para: " + novoStatus + "?",
